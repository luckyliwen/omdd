fd.string = {
	...
	/**
	 * Get the most matched string from an array. If several have the same value, then all return. 
	 * @param  {[type]} testStr [description]
	 * @param  {[type]} aArr    [description]
	 * @return {[type]}         [description]
	 */
	getSimilarFromArray: function( testStr, aCandidate ) {
	    var maxSimilarity = 0.4;
	    var aRet = [];

	    for (var i=0; i < aCandidate.length; i++) {
	    	var  candidate = aCandidate[i];
	    	var similarity = this.getSimilarity( testStr, candidate);
	    	if (similarity > maxSimilarity) {
	    		aRet = [ candidate ];
	    		maxSimilarity = similarity;
	    	} else if (similarity == maxSimilarity) {
	    		//several candidate have the same weight 
	    		aRet.push( candidate);
	    	}
	    }
	...
	getSimilarity: function(sa1, sa2) {
		...
		//if is NaN then change to 0 for simple
		if ( isNaN(similarity))
			similarity = 0;
